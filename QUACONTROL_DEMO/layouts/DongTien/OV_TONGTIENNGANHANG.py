...
def gen_layout():
    ...
    title = 'TỔNG TIỀN NGÂN HÀNG'
    color = '#008080'
    icon = 'cashbank.svg'

    layout = create_card(title,'ov_112',color,icon)
           
    return layout


@app_DONG_TIEN.callback(
    Output("ov_112", "children"),
...
def UPDATE_OV_GT(date_wh,LOAITIEN,THUCHI,TIENVAY,DT_VNH,data_VNH):
    ...
    df = DB_DONGTIEN.GET_DONGTIEN(('ov_112',date_wh,label,value))
    return df['TienNganHang'].values[0]